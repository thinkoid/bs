...
options_t::options_t (int argc, char** argv) {
    ...
    config.add_options ()
        ...
        ("alpha,a", po::value< double > ()->default_value (.95),
         "alpha, a weight.")
...
static void
process_adaptive_learning (cv::VideoCapture& cap, const options_t& opts) {
    ...
    bs::adaptive_learning subtractor (
        background,
    ...
    for (auto& frame : bs::getframes_from (cap)) {
        ...
        if (display)
            imshow ("Adaptive background learning", mask);
...
int main (int argc, char** argv) {
    ...
    return run_with (process_adaptive_learning, global_options ()), 0;
}
