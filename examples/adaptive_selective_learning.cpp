...
process_adaptive_selective_background (
    cv::VideoCapture& cap, const options_t& opts) {
    ...
    bs::adaptive_selective_learning subtractor (
        background,
    ...
    for (auto& frame : bs::getframes_from (cap)) {
        ...
        if (display)
            imshow ("Adaptive selective background learning", mask);
...
int main (int argc, char** argv) {
    ...
    return run_with (
        process_adaptive_selective_background, global_options ()), 0;
}
