...
options_t::options_t (int argc, char** argv) {
    ...
    ("measure,m", po::value< std::vector< double > > ()->multitoken ()
     ->default_value (std::vector< double > { .6, .3, .1 }),
...
process_fuzzy_choquet (cv::VideoCapture& cap, const options_t& opts) {
    ...
    for (auto& frame : bs::getframes_from (cap)) {
        ...
        const auto mask = fuzzy_choquet (frame);

        if (display)
            imshow ("Fuzzy Choquet filter", mask);
...
int main (int argc, char** argv) {
    ...
    return run_with (process_fuzzy_choquet, global_options ()), 0;
}
