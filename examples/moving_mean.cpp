...
static void
process_moving_mean (cv::VideoCapture& cap, const options_t& opts) {
    ...
    for (auto& frame : bs::getframes_from (cap)) {
        ...
        if (display)
            imshow ("Moving mean background subtraction", mask);
...
int main (int argc, char** argv) {
    ...
    return run_with (process_moving_mean, global_options ()), 0;
}
