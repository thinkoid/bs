...
options_t::options_t (int argc, char** argv) {
    ...
    config.add_options ()
        ...
        ("weights,w", po::value< std::vector< double > > ()->multitoken ()
         ->default_value (std::vector< double >{ .33, .33, .33 }),
         "weights(3) to assign to the previous 3 frames")
...
static void
process_windowed_mean (cv::VideoCapture& cap, const options_t& opts) {
    const bool display = opts.have ("display");

    bs::windowed_mean subtractor (
    ...
    for (auto& frame : bs::getframes_from (cap)) {
        ...
        if (display)
            imshow ("Windowed(3) moving mean difference", mask);
...
int main (int argc, char** argv) {
    ...
    return run_with (process_windowed_mean, global_options ()), 0;
}
