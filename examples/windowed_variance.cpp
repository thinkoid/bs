...
options_t::options_t (int argc, char** argv) {
    ...
    config.add_options ()
        ...
        ("weights,w", po::value< std::vector< double > > ()->multitoken ()
         ...
         "weights(3) for frames in framebuffer")
...
static void
process_windowed_variance (cv::VideoCapture& cap, const options_t& opts) {
    const bool display = opts.have ("display");

    bs::windowed_variance subtractor (
    ...
    for (auto& frame : bs::getframes_from (cap)) {
        ...
        if (display)
            imshow ("Windowed variance difference", mask);
...
int main (int argc, char** argv) {
    ...
    return run_with (process_windowed_variance, global_options ()), 0;
}
